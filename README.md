...
- Metadata pertaining to a specific model release (this is specified by an optional `model_release` parameter, which defaults to latest, as configured via `config.ts`):
  ...
  - The shapefile source for the model release
...
Each model output directory must contain a `metadata.json` file documenting its version and dependencies on a STAVE data release and shapefile source. The example model outputs generator creates this file; it must be provided alongside real model outputs.

### Test fixtures

To avoid making tests dependent on potentially changing data, the integration tests use small fixtures (relatively small compared to real data). They shouldn't need frequently updating unless the data schema itself is changing. These are generated by a script, which is configured by [./tests/fixtures/fixture-config.json](./tests/fixtures/fixture-config.json):
